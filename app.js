...
.hero::before {
  ...
  bottom: 0;
  background: 
    radial-gradient(circle at 20% 80%, rgba(var(--color-teal-500-rgb), 0.15), transparent 50%),
    radial-gradient(circle at 80% 20%, rgba(var(--color-teal-500-rgb), 0.1), transparent 50%);
  pointer-events: none;
}
...
.example-card.night-mode {
  background: var(--color-bg-8);
}

.example-card.day-mode {
...
.feature-card::before {
  ...
  width: 200%;
  height: 200%;
  background: radial-gradient(circle, rgba(var(--color-teal-500-rgb), 0.05), transparent 70%);
  transform: scale(0);
  transition: transform var(--duration-normal) var(--ease-standard);
}
...
.form-control.success {
  ...
  box-shadow: 0 0 0 2px rgba(var(--color-success-rgb), 0.1);
}
